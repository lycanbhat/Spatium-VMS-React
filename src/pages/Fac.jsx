...
export default function Fac() {
  ...
  const [pageTitle, setPageTitle] = useState("Facilities");
  const [tableHeaders, setTableHeaders] = useState([
    // { label: "ID", key: "username" },
    { label: "Name", key: "email" },
    { label: "City", key: "memberSince" },
    { label: "Zone", key: "memberSince" },
    { label: "", key: "actions" },
  ]);
...
const TRow = ({ data, fetchFacilities }) => {
  ...
  useEffect(()=>{
    async function getCityNameById(id) {
      ...
      const city = data.results.find(city => city.id === id);
      // return city ? city.name : undefined; // Handle case where ID not found
      setCity(city.name)
    }
    async function getZoneNameById(id) {
      const {data} = await makeApiCall('GET',`v1/admin/zone/?page_size=500`)
      const city = data.results.find(city => city.id === id);
      // return city ? city.name : undefined; // Handle case where ID not found
      setZone(city.name)
    }
    getCityNameById(data.city)
    getZoneNameById(data.zone)
  },[])

  const deleteFac = async () => {
    try {
      await makeApiCall("DELETE", `${"v1/admin/facility/"}${data.id}/`);
      fetchFacilities();
      closeDelete();
    } catch (error) {
      console.error(error);
  ...
  return (
    <>
      <tr className="border-t border-b border-dark-200 h-10 mx-1 hover:bg-light-500">
        ...
        <td className="pl-2">{city_}</td>
        <td className="pl-2">{zone_}</td>
